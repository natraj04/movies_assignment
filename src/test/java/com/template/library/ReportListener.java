...
public class ReportListener implements ISuiteListener, ITestListener {
    ...
    public synchronized void onTestStart(ITestResult result) {
        ...
        if(type.equalsIgnoreCase("excel")) {
        	extentTest = extent.createTest(result.getMethod().getMethodName()+" | "+result.getParameters()[0],result.getMethod().getDescription());
        } else {
        	extentTest = extent.createTest(result.getMethod().getMethodName());
        }       
        System.out.println("Test Name:- ------ "+result.getTestClass().getXmlTest().getName());
        extentTest.assignCategory(result.getTestClass().getXmlTest().getName()); 
        test.set(extentTest);
        if(type.equalsIgnoreCase("excel")) 
        	test.get().info(result.getMethod().getMethodName()+" | "+result.getParameters()[0] + " test is started");
        else 
    ...
    public synchronized void onTestSuccess(ITestResult result) {
        ...
        if (screenshot.equalsIgnoreCase("pass") || screenshot.equalsIgnoreCase("default") || screenshot.equalsIgnoreCase("yes")) {
    ...
    public synchronized void onTestFailure(ITestResult result) {
        ...
        if (screenshot.equalsIgnoreCase("default") || screenshot.equalsIgnoreCase("fail") || screenshot.equalsIgnoreCase("yes")) {
			String file = takeScreenShot(result);
			if (!file.equals("")) {
				try {
					test.get().addScreenCaptureFromPath(new File(file).getName(), result.getMethod().getMethodName());
		...
		try {
			if (!(WebDriverRunner.getWebDriver() == null)) {
				...
				if(System.getProperty("os.name").toLowerCase().contains("win"))
					filePath=System.getProperty("user.dir") + "\\target\\surefire-reports\\"+ result.getMethod().getMethodName() + " " + timeStamp + ".png";
				else 
					filePath =System.getProperty("user.dir") + "/target/surefire-reports/" + result.getMethod().getMethodName() + " " + timeStamp + ".png";
